# -*- coding:utf-8 *-*
#######
#this script uses to get the deduplication of fiu trace homes
#usage: python fiu_dedup_homes.py directory_of_homes file_num
#
import os
import sys
...
out_file = open("stat_homes.txt", "w+")
		...
		if len(line):
			...
			if(line_split[5] == 'W' or line_split[5] == 'w' ):
				...
				for i in xrange(block_num/8):
					digest_str = trace_digest[256*i:256*i+256]
...
def trace_dir(path, file_num):
	if os.path.isdir(path):
		...
		for root, dirs, files in list_dirs:
			print 'Begin to get fiu_trace homes hash_key stat'
			if(file_num == 0):
				file_num = len(files)
			for i in xrange(1, (file_num + 1)):
				trace_file_name = 'homes-110108-112108.' + str(i) + '.blkparse'
				print trace_file_name
				get_trace_reference(os.path.join(root,trace_file_name))
			print 'End fiu_trace homes stat\n'

			print 'Begin to convert fiu_trace homes to disksim trace + reference count'
			for i in xrange(1, (file_num + 1)):
				trace_file_name = 'homes-110108-112108.' + str(i) + '.blkparse'
				print trace_file_name
				trace_file(os.path.join(root,trace_file_name))
			print 'End fiu_trace homes reference count\n'
	else:
